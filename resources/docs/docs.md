...
The `home-routes` handler in the `sente-example.routes.home` namespace
...
The `home-page` function will in turn call the `sente-example.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/sente_example/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `sente-example.handler` namespace:
...
Request middleware functions are located under the `sente-example.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `sente-example.dev-middleware` namespace found in
